...
export async function syncNow(): Promise<void> {
	...
	try {
		...
		// Manual sync should still pull latest cloud state even if push fails.
		pushError = err;
		console.warn('[sync] push failed during syncNow, continuing with pull', err);
	}
	await pullFromCloud(user.id);
	if (pushError) {
		// Surface non-blocking push issue in console/status while keeping pull success path usable.
		syncStatus.setOk();
	}
}
